...
static void
EscCmacAes_LeftShiftBlock(
    Esc_UINT8 block[] );

static void
EscCmacAes_ApplySubkey(
    EscCmacAes_ContextT* ctx );

static void
EscCmacAes_EncryptBlock(
    EscCmacAes_ContextT* ctx );

static void
EscCmacAes_UpdateLastBlock(
    EscCmacAes_ContextT* ctx );
    ...
    Esc_UINT8 block[] )
{
    ...
    for (i = 0U; i < (EscAes_BLOCK_BYTES - 1U); i++)
    {
        Esc_UINT8 t;
    ...
    block[EscAes_BLOCK_BYTES - 1U] = (Esc_UINT8)(block[EscAes_BLOCK_BYTES - 1U] << 1);
}
...
EscCmacAes_EncryptBlock(
    EscCmacAes_ContextT* ctx )
{
    EscAes_EncryptBlock( &ctx->aesCtx, ctx->block, ctx->block );
    ctx->blockLen = 0U;
}
...
EscCmacAes_ApplySubkey(
    EscCmacAes_ContextT* ctx )
{
    static const Esc_UINT8 ALL_ZERO[EscAes_BLOCK_BYTES] =     /* For CMAC, the IV is 000000... */
    {
        0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U
    };
    static const Esc_UINT8 CMAC_RB = 0x87U; /* R_B value of chapter 5.3 */
    Esc_UINT8 k[EscAes_BLOCK_BYTES];      /* Contains L, K1 and K2 */
    Esc_UINT8 i;

    /*** Subkey generation, chapter 6.1. Calculate the k1 and k2 ***/

    /* 1. K1 = L = ciph(0) */
    EscAes_EncryptBlock( &ctx->aesCtx, ALL_ZERO, k );

    /* 2. If MSB1(L) = 0, then K1 = L << 1;
       Else K1 = (L << 1) exor Rb */
    if ( (k[0] & 0x80U) == 0U )
    {
        EscCmacAes_LeftShiftBlock( k );    /* K1 = L << 1 */
    }
    else
    {
        EscCmacAes_LeftShiftBlock( k );    /* K1 = L << 1 */
        k[EscAes_BLOCK_BYTES - 1U] ^= CMAC_RB;    /* K1 = (L << 1) exor Rb */
    }

    /* If not a complete block, calculate K2 */
    if (ctx->blockLen != EscAes_BLOCK_BYTES)
    {
        /* 3. If MSB1(K1) = 0, then K2 = K1 << 1;
           Else K2 = (K1 << 1) exor Rb. */
        if ( (k[0] & 0x80U) == 0U )
        {
            EscCmacAes_LeftShiftBlock( k );    /* K2 = K1 << 1 */
        }
        else
        {
            EscCmacAes_LeftShiftBlock( k );    /* K2 = K1 << 1 */
            k[EscAes_BLOCK_BYTES - 1U] ^= CMAC_RB;    /* K2 = (K1 << 1) exor Rb */
        }
    }
    ...
    EscCmacAes_ContextT* ctx )
{
    ...
    /* Apply subkey */
    EscCmacAes_ApplySubkey( ctx );

    /* Encrypt last block */
    EscCmacAes_EncryptBlock( ctx );
}
    ...
    Esc_UINT32 keyLength )
{
    ...
    else if ( ((keyLength != 16U) && (keyLength != 24U) && (keyLength != 32U)) ||
              (keyLength > EscAes_MAX_KEY_BYTES) )
    {
    ...
    {
        /* set AES key */
        EscAes_Init( &ctx->aesCtx, (Esc_UINT16)(keyLength * 8U), key );
    ...
    Esc_UINT32 messageLength )
{
         ...
         ( (message != Esc_NULL_PTR) || (messageLength == 0U) ) ) /* Allow NULL pointer if length is zero */
    {
        ...
        while (bytesLeft > 0U)
        {
            ...
            /* fill block with remaining bytes, blockLen is now smaller than EscAes_BLOCK_BYTES */
            if ( bytesLeft >= ( (Esc_UINT32)EscAes_BLOCK_BYTES - (Esc_UINT32)ctx->blockLen ) )
            {
                bytesToFill = (Esc_UINT8)(EscAes_BLOCK_BYTES - ctx->blockLen);
            }
            else
            {
    ...
    Esc_UINT8 macLength )
{
    ...
    {
        if ( (macLength <= EscAes_BLOCK_BYTES) && (macLength > 0U) )
        {
            /* 4. Process residual block if any */
            EscCmacAes_UpdateLastBlock( ctx );
    ...
    Esc_UINT8 macLength )
{
    ...
    returnCode = EscCmacAes_Init( &ctx, key, keyLength );
    if (returnCode == Esc_NO_ERROR)
    {
        returnCode = EscCmacAes_Update( &ctx, message, messageLength );
        if (returnCode == Esc_NO_ERROR)
        {
            returnCode = EscCmacAes_Finish( &ctx, mac, macLength );
        }
    }

    /* Zeroize AES-CMAC context as it contains key rounds */
