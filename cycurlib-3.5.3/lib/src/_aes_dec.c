...
/* T-tables, see aes.spec.311.pdf, BigEndian! */

#    define INVERSE_T_TABLE_0_DATA                                                                              \
    {                                                                                                           \
...
#if EscAes_USE_T_TABLES_FULL == 1

#    define INVERSE_T_TABLE_1_DATA                                                                              \
    {                                                                                                           \
 ...
 *********************************************************/
#if ( EscAes_USE_T_TABLES_SMALL == 1 )
...
EscAes_DecryptBlock(
    ...
    Esc_UINT8 plain[] )
{
    static const Esc_UINT8 EscAes_INVERSE_SBOX[256U] = INVERSE_S_BOX_DATA;
    static const Esc_UINT32 EscAes_INVERSE_T_TABLE_0[256U] = INVERSE_T_TABLE_0_DATA;

    const Esc_UINT32* round_key;    /* pointer to round key                */
    Esc_UINT32 cx[4];               /* state array columns, cx for input   */
    Esc_UINT32 cy[4];               /* state array columns, cy for output  */
    Esc_UINT8 round;                /* current round index */
    Esc_UINT8 i;
    Esc_ASSERT( ctx != 0 );
    Esc_ASSERT( cipher != 0 );
    Esc_ASSERT( plain != 0 );
    /* get last transformed decryption key for the equivalent inverse cipher */
    round_key = &ctx->dec_keys[ctx->nr * 4U];

    /* read 16 cipher text bytes into four 32-bit words
       (FIPS-197: state = in and AddRoundKey)
       XOR it with enc_round_key[43..40] */
    for (i = 0U; i < 4U; i++)
    {
        Esc_UINT32 idx;
        idx = ( (Esc_UINT32)i ) << 2;
        cx[i] = EscAes_Octets2Uint32( cipher, idx );
        cx[i] ^= round_key[i];
    }

    /* do decryption rounds Nr-1..0 */
    /* inverse S-box, inverse ShiftRows() via input structure, inverseMixCol, XOR round_key */
    for (round = 0U; round < ctx->nr; round++)
    {
        Esc_UINT8 roundIdx;

        roundIdx = (Esc_UINT8)( (ctx->nr - 1U) - round );

        round_key = &ctx->dec_keys[roundIdx * 4U];

        if (roundIdx > 0U)
        {
            /* T-table implementation. Instead of four 1kB tables we only use one
               table and rotate 8 bits to the right (rotate left 24) to compute
               the other ones on the fly */
            cy[0] = EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_0( cx[0] )] ^          /* T0 */
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_1( cx[3] )], 24U ) ^       /* T1 */
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_2( cx[2] )], 16U ) ^       /* T2 */
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_3( cx[1] )], 8U );         /* T3 */
            cy[1] = EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_0( cx[1] )] ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_1( cx[0] )], 24U ) ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_2( cx[3] )], 16U ) ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_3( cx[2] )], 8U );
            cy[2] = EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_0( cx[2] )] ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_1( cx[1] )], 24U ) ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_2( cx[0] )], 16U ) ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_3( cx[3] )], 8U );
            cy[3] = EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_0( cx[3] )] ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_1( cx[2] )], 24U ) ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_2( cx[1] )], 16U ) ^
                EscAes_RotateLeft32( EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_3( cx[0] )], 8U );
        }
        else
        {
            /* Last round: Only InvSubBytes and InvShiftRows using the small inverse sbox */
            for (i = 0U; i < 4U; i++)
            {
                cy[i] =
                    ( ( (Esc_UINT32)(EscAes_INVERSE_SBOX[EscAes_BYTE_0( cx[i] )]) ) << 24 ) ^
                    ( ( (Esc_UINT32)(EscAes_INVERSE_SBOX[EscAes_BYTE_1( cx[(i + 3U) & 0x3U] )]) ) << 16 ) ^
                    ( ( (Esc_UINT32)(EscAes_INVERSE_SBOX[EscAes_BYTE_2( cx[(i + 2U) & 0x3U] )]) ) << 8 ) ^
                    ( ( (Esc_UINT32)(EscAes_INVERSE_SBOX[EscAes_BYTE_3( cx[(i + 1U) & 0x3U] )]) ) );
            }
        }
        ...
        for (i = 0U; i < 4U; i++)
        {
            cx[i] = cy[i] ^ round_key[i];
        }
    }

    /* write result into plain text byte array */
    EscAes_State2Uint32Array( plain, cx );
}

#elif ( EscAes_USE_T_TABLES_FULL == 1 )
    ...
    Esc_UINT8 plain[] )
{
    ...
    static const Esc_UINT32 EscAes_INVERSE_T_TABLE_0[256U] = INVERSE_T_TABLE_0_DATA;
    static const Esc_UINT32 EscAes_INVERSE_T_TABLE_1[256U] = INVERSE_T_TABLE_1_DATA;
    static const Esc_UINT32 EscAes_INVERSE_T_TABLE_2[256U] = INVERSE_T_TABLE_2_DATA;
    static const Esc_UINT32 EscAes_INVERSE_T_TABLE_3[256U] = INVERSE_T_TABLE_3_DATA;
    ...
    for (round = 0U; round < ctx->nr; round++)
    {
        ...
        if (roundIdx > 0U)
        {
            ...
            cy[0] = EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_0( cx[0] )] ^          /* T0 */
                EscAes_INVERSE_T_TABLE_1[EscAes_BYTE_1( cx[3] )] ^              /* T1 */
                EscAes_INVERSE_T_TABLE_2[EscAes_BYTE_2( cx[2] )] ^              /* T2 */
                EscAes_INVERSE_T_TABLE_3[EscAes_BYTE_3( cx[1] )];               /* T3 */
            cy[1] = EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_0( cx[1] )] ^
                EscAes_INVERSE_T_TABLE_1[EscAes_BYTE_1( cx[0] )] ^
                EscAes_INVERSE_T_TABLE_2[EscAes_BYTE_2( cx[3] )] ^
                EscAes_INVERSE_T_TABLE_3[EscAes_BYTE_3( cx[2] )];
            cy[2] = EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_0( cx[2] )] ^
                EscAes_INVERSE_T_TABLE_1[EscAes_BYTE_1( cx[1] )] ^
                EscAes_INVERSE_T_TABLE_2[EscAes_BYTE_2( cx[0] )] ^
                EscAes_INVERSE_T_TABLE_3[EscAes_BYTE_3( cx[3] )];
            cy[3] = EscAes_INVERSE_T_TABLE_0[EscAes_BYTE_0( cx[3] )] ^
                EscAes_INVERSE_T_TABLE_1[EscAes_BYTE_1( cx[2] )] ^
                EscAes_INVERSE_T_TABLE_2[EscAes_BYTE_2( cx[1] )] ^
                EscAes_INVERSE_T_TABLE_3[EscAes_BYTE_3( cx[0] )];
        }
        else
        {
            /* Last round: Only InvSubBytes and InvShiftRows using the small inverse sbox */
    ...
    Esc_UINT8 plain[] )
{
    ...
    /* get last round key */
    round_key = &ctx->round_keys[ctx->nr * 4U];
    ...
    for (round = 0U; round < ctx->nr; round++)
    {
        ...
        round_key = &ctx->round_keys[roundIdx * 4U];

        /* InvShiftRows, InvSubBytes and AddRoundKey */
        for (i = 0U; i < 4U; i++)
        {
            cy[i] = round_key[i];
            cy[i] ^= ( (Esc_UINT32)(EscAes_INVERSE_SBOX[EscAes_BYTE_0( cx[i] )]) ) << 24;
            cy[i] ^= ( (Esc_UINT32)(EscAes_INVERSE_SBOX[EscAes_BYTE_1( cx[(i + 3U) & 0x3U] )]) ) << 16;
            cy[i] ^= ( (Esc_UINT32)(EscAes_INVERSE_SBOX[EscAes_BYTE_2( cx[(i + 2U) & 0x3U] )]) ) << 8;
            cy[i] ^= ( (Esc_UINT32)(EscAes_INVERSE_SBOX[EscAes_BYTE_3( cx[(i + 1U) & 0x3U] )]) );
        }

        /* InvMixColumns */
        if (roundIdx > 0U)
        {
            cx[0] = EscAes_InvMixColumns( cy[0] );
            cx[1] = EscAes_InvMixColumns( cy[1] );
            cx[2] = EscAes_InvMixColumns( cy[2] );
            cx[3] = EscAes_InvMixColumns( cy[3] );
        }
    }                           /* for rounds Nr-1..0 */

    /* write result into plain text byte array */
    /*lint -e{772} cy is completely initialized at least in the first for loop . */
    EscAes_State2Uint32Array( plain, cy );
}
