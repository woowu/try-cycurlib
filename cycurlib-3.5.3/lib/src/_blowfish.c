...
static Esc_UINT32
EscBfish_OctetsToUint32(
    const Esc_UINT8 octets[] );

static void
EscBfish_Uint32ToOctets(
    Esc_UINT8 octets[],
    Esc_UINT32 word );
...
/**
 Convert four octets in big endian format to Esc_UINT32.
 */
static Esc_UINT32
EscBfish_OctetsToUint32(
    const Esc_UINT8 octets[] )
{
    ...
    v = ( ( (Esc_UINT32)octets[0U] << 24U ) |
          ( (Esc_UINT32)octets[1U] << 16U ) |
          ( (Esc_UINT32)octets[2U] << 8U ) |
          ( (Esc_UINT32)octets[3U] )
          );
...
/**
 Convert four octets from Esc_UINT32 to big endian format.
 */
static void
EscBfish_Uint32ToOctets(
    Esc_UINT8 octets[],
    Esc_UINT32 word )
{
    ...
    octets[3U] = (Esc_UINT8)(v & 0xFFU);
    v >>= 8U;
    octets[2U] = (Esc_UINT8)(v & 0xFFU);
    v >>= 8U;
    octets[1U] = (Esc_UINT8)(v & 0xFFU);
    v >>= 8U;
    octets[0U] = (Esc_UINT8)(v & 0xFFU);
}

/**
 * Performs S-box substitution function F.
 */
static Esc_UINT32
EscBfish_F(
    const EscBfish_KeySchedT* sched,
    ...
    const Esc_UINT8 bfishBlock[] )
{
    Esc_ASSERT( pl != 0 );
    Esc_ASSERT( pr != 0 );
    Esc_ASSERT( bfishBlock != 0 );

    *pl = EscBfish_OctetsToUint32( bfishBlock );
    *pr = EscBfish_OctetsToUint32( &bfishBlock[4U] );
}
...
EscBfish_MergeBlocks(
    ...
    Esc_UINT32 pr )
{
    EscBfish_Uint32ToOctets( bfishBlock, pl );
    EscBfish_Uint32ToOctets( &bfishBlock[4U], pr );
}

/**
 Performs Blowfish encryption algorithm.
 */
void
EscBfish_Encrypt(
    const EscBfish_KeySchedT* sched,
    ...
    Esc_UINT32* pr )
{
    ...
    /** Performs all Feistel rounds  except the last one. */
    for (i = 0U; i < (EscBfish_NUM_ROUNDS - 1U); i++)
    {
        left = left ^ sched->p[i];
        right = EscBfish_F( sched, &left ) ^ right;

        /* Swap left and right using XOR */
        left ^= right;
        right ^= left;
        left ^= right;
    }

    /** Performs the last Feistel round. */
    left = left ^ sched->p[EscBfish_NUM_ROUNDS - 1U];
    right = EscBfish_F( sched, &left ) ^ right;

    *pr = right ^ sched->p[EscBfish_NUM_ROUNDS];
    *pl = left ^ sched->p[EscBfish_NUM_ROUNDS + 1U];
}

/**
 * Performs Blowfish decryption algorithm.
 */
void
EscBfish_Decrypt(
    const EscBfish_KeySchedT* sched,
    ...
    Esc_UINT32* pr )
{
    ...
    /** Performs all Feistel rounds except the last one. */
    for (i = (Esc_UINT8)(EscBfish_NUM_ROUNDS + 1U); i > 2U; i--)
    {
        left = left ^ sched->p[i];
        right = EscBfish_F( sched, &left ) ^ right;
...
{
    /** Blowfish initial P-array */
    static const Esc_UINT32 EscBfish_P[18U] =
    {
    ...
    /** Blowfish initial S-boxes */
    static const Esc_UINT32 EscBfish_SBOX[EscBfish_NUM_SBOX][EscBfish_SIZE_SBOX] =
    {
        {
    ...
    Esc_ASSERT( sched != 0 );
    Esc_ASSERT( key != 0 );
    Esc_ASSERT( keyLen >= EscBfish_MIN_KEY_BYTES );
    Esc_ASSERT( keyLen <= EscBfish_MAX_KEY_BYTES );
    ...
    for (i = 0U; i < 4U; i++)
    {
        for (j = 0U; j < 256U; j++)
        {
            sched->s[i][j] = EscBfish_SBOX[i][j];
        }
    }

    /** Initialize P-array */
    j = 0U;
    for (i = 0U; i < (EscBfish_NUM_ROUNDS + 2U); i++)
    {
        left = 0U;
        for (k = 0U; k < 4U; k++)
        {
        ...
        sched->p[i] = EscBfish_P[i] ^ left;
    }

    /** Compute P-array (9 iterations). */
    left = 0U;
    right = 0U;
    for (i = 1U; i <= (EscBfish_NUM_ROUNDS + 1U); i += 2U)
    {
        EscBfish_Encrypt( sched, &left, &right );
        sched->p[i - 1U] = left;
        sched->p[i] = right;
    }

    /** Compute S-boxes (4 * 128 = 512 iterations). */
    for (i = 0U; i < 4U; i++)
    {
        for (j = 1U; j <= 255U; j += 2U)
        {
            EscBfish_Encrypt( sched, &left, &right );
            sched->s[i][j - 1U] = left;
            sched->s[i][j] = right;
