...
EscAes_EncryptBlock(
    ...
    Esc_UINT8 cipher[] )
{
    static const Esc_UINT32 EscAes_FORWARD_T_TABLE_0[256] = FORWARD_T_TABLE_0_DATA;
    ...
    round_key = &ctx->round_keys[0];

    /* read 16 cipher text bytes into four 32-bit words
       (FIPS-197: state = in and AddRoundKey)
       XOR it with enc_round_key[3..0] */
    for (i = 0U; i < 4U; i++)
    {
        Esc_UINT32 idx;
        idx = ( (Esc_UINT32)i ) << 2;
        cx[i] = EscAes_Octets2Uint32( plain, idx );
        cx[i] ^= round_key[i];
    }

    /* do encryption rounds 0..Nr-1 */
    /* S-box, ShiftRows() via input structure, MixCol, XOR round_key */
    for (round = 0U; round < ctx->nr; round++)
    {
        round_key = &ctx->round_keys[(round + 1U) * 4U];

        if ( round < (ctx->nr - 1U) )
        {
            /* T-table implementation (see aes.spec.311.pdf). Instead of four 1kB
               tables we only use one table and rotate 8 bits to the right
               (rotate left 24) to compute the other ones on the fly */
            cy[0] = EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_0( cx[0] )] ^          /* T0 */
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_1( cx[1] )], 24U ) ^       /* T1 */
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_2( cx[2] )], 16U ) ^       /* T2 */
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_3( cx[3] )], 8U );       /* T3 */
            cy[1] = EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_0( cx[1] )] ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_1( cx[2] )], 24U ) ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_2( cx[3] )], 16U ) ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_3( cx[0] )], 8U );
            cy[2] = EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_0( cx[2] )] ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_1( cx[3] )], 24U ) ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_2( cx[0] )], 16U ) ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_3( cx[1] )], 8U );
            cy[3] = EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_0( cx[3] )] ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_1( cx[0] )], 24U ) ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_2( cx[1] )], 16U ) ^
                EscAes_RotateLeft32( EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_3( cx[2] )], 8U );
        }
        else
        {
            /* Last round: SubBytes, ShiftRows only w/o T-table */
            for (i = 0U; i < 4U; i++)
            {
                cy[i] =
                    ( ( (Esc_UINT32)(EscAes_FORWARD_SBOX[EscAes_BYTE_0( cx[i] )]) ) << 24 ) ^
                    ( ( (Esc_UINT32)(EscAes_FORWARD_SBOX[EscAes_BYTE_1( cx[(i + 1U) & 0x3U] )]) ) << 16 ) ^
                    ( ( (Esc_UINT32)(EscAes_FORWARD_SBOX[EscAes_BYTE_2( cx[(i + 2U) & 0x3U] )]) ) << 8 ) ^
                    ( ( (Esc_UINT32)(EscAes_FORWARD_SBOX[EscAes_BYTE_3( cx[(i + 3U) & 0x3U] )]) ) );
            }
        }
    ...
    /* write result into cipher text byte array */
    EscAes_State2Uint32Array( cipher, &cx[0U] );
}
...
{
    static const Esc_UINT32 EscAes_FORWARD_T_TABLE_0[256] = FORWARD_T_TABLE_0_DATA;
    static const Esc_UINT32 EscAes_FORWARD_T_TABLE_1[256] = FORWARD_T_TABLE_1_DATA;
    static const Esc_UINT32 EscAes_FORWARD_T_TABLE_2[256] = FORWARD_T_TABLE_2_DATA;
    static const Esc_UINT32 EscAes_FORWARD_T_TABLE_3[256] = FORWARD_T_TABLE_3_DATA;
    ...
    for (round = 0U; round < ctx->nr; round++)
    {
        ...
        if ( round < (ctx->nr - 1U) )
        {
            /* T-table implementation (see aes.spec.311.pdf). */
            cy[0] = EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_0( cx[0] )] ^          /* T0 */
                EscAes_FORWARD_T_TABLE_1[EscAes_BYTE_1( cx[1] )] ^       /* T1 */
                EscAes_FORWARD_T_TABLE_2[EscAes_BYTE_2( cx[2] )] ^       /* T2 */
                EscAes_FORWARD_T_TABLE_3[EscAes_BYTE_3( cx[3] )];        /* T3 */
            cy[1] = EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_0( cx[1] )] ^
                EscAes_FORWARD_T_TABLE_1[EscAes_BYTE_1( cx[2] )] ^
                EscAes_FORWARD_T_TABLE_2[EscAes_BYTE_2( cx[3] )] ^
                EscAes_FORWARD_T_TABLE_3[EscAes_BYTE_3( cx[0] )];
            cy[2] = EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_0( cx[2] )] ^
                EscAes_FORWARD_T_TABLE_1[EscAes_BYTE_1( cx[3] )] ^
                EscAes_FORWARD_T_TABLE_2[EscAes_BYTE_2( cx[0] )] ^
                EscAes_FORWARD_T_TABLE_3[EscAes_BYTE_3( cx[1] )];
            cy[3] = EscAes_FORWARD_T_TABLE_0[EscAes_BYTE_0( cx[3] )] ^
                EscAes_FORWARD_T_TABLE_1[EscAes_BYTE_1( cx[0] )] ^
                EscAes_FORWARD_T_TABLE_2[EscAes_BYTE_2( cx[1] )] ^
                EscAes_FORWARD_T_TABLE_3[EscAes_BYTE_3( cx[2] )];
        }
        else
        {
            /* Last round: SubBytes, ShiftRows only w/o T-table */
    ...
    Esc_CLEAR_LOCAL_ARRAY(cx);
    Esc_CLEAR_LOCAL_ARRAY(cy);
    ...
    Esc_UINT8 cipher[] )
{
    ...
    for (round = 0U; round < ctx->nr; round++)
    {
        ...
        for (i = 0U; i < 4U; i++)
        {
            cy[i] = ( (Esc_UINT32)(EscAes_FORWARD_SBOX[EscAes_BYTE_0( cx[i] )]) ) << 24;
            cy[i] ^= ( (Esc_UINT32)(EscAes_FORWARD_SBOX[EscAes_BYTE_1( cx[(i + 1U) & 0x3U] )]) ) << 16;
            cy[i] ^= ( (Esc_UINT32)(EscAes_FORWARD_SBOX[EscAes_BYTE_2( cx[(i + 2U) & 0x3U] )]) ) << 8;
            cy[i] ^= ( (Esc_UINT32)(EscAes_FORWARD_SBOX[EscAes_BYTE_3( cx[(i + 3U) & 0x3U] )]) );
        }

        /* Round 0..EscAes_Nr MixColumns()  */
        if ( round < (ctx->nr - 1U) )
        {
            cy[0] = EscAes_MixColumns( cy[0] );
            cy[1] = EscAes_MixColumns( cy[1] );
            cy[2] = EscAes_MixColumns( cy[2] );
            cy[3] = EscAes_MixColumns( cy[3] );
        }
